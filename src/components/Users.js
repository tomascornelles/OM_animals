...
class Users extends React.Component {
  ...
  _list = () => {
    ...
    users = users.filter(
      (user) => user.animals.indexOf(animal) >= 0 && user.isActive
    );
    ...
    return (
      <div className="list">
        {usersFiltered.map((user) => (
          <details key={user.id}>
            <summary>
              {user.name.given} {user.name.surname} (age {user.age}){" "}
            ...
            {user.animals.map((animal) => (
              <button
                ...
                onClick={() => {
                  this.props.clickHandler("users", animal, 10);
                }}
              >
                {animal}
              </button>
            ))}
            <button className="danger" onClick={() => this.remove(user.id)}>
              Remove user
            </button>
          </details>
        ))}
        {limit === 10 && (
          <p>
            <button
              onClick={() => {
                this.props.clickHandler("users", animal, 25);
              }}
            >
              Show more
            </button>
  ...
  render() {
    if (this.props.editUser === "new") {
      return <NewUser clickHandler={this.props.clickHandler} />;
    } else {
      return this.list();
    }
  }
